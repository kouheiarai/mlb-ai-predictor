...
[`launch/LAUNCH.md`](launch/LAUNCH.md) に、beehiiv の設定値、登録ページの文面、ウェルカム
メール、最初の告知投稿、見るべき指標をそのまま使える形で置いてある。
...
`.html` は**純 ASCII** で書き出している。`¥` や em dash（—）は UTF-8 では複数バイトになるため、
Windows のメモ帳や Excel のように CP932 を既定とするアプリで開くと `ﾂ･` `窶` のように化ける。
そこで非 ASCII 文字はすべて数値文字参照（`&#165;` など）に変換してある。

- ファイルはどのエンコーディングで開いても壊れない
- ブラウザと beehiiv は元の文字として描画する
- **エディタ上で `&#165;` と見えていても、そのまま貼って問題ない**
