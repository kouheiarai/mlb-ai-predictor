...
枯渇した場合、`main.py` は更新をスキップして**正常終了**します。翌月のリセットまで必ず起きる
想定内の停止であり、失敗として扱うと実行のたびに通知メールが飛ぶだけだからです。
Actions の実行画面には警告注釈が残り、公開ファイルは最後に成功した内容のまま据え置かれます
（壊れたデータは公開されません）。

このため、**Actions から失敗通知が届いたときは本物の不具合**です。
