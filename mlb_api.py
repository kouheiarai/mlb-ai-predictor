...
def fetch_pitcher_season_stats(pitcher_id: int | None, season: int) -> dict[str, Any]:
    if not pitcher_id:
        return {"pitcher_id": None, "era": None, "whip": None, "strikeout_walk_ratio": None, "innings_pitched": None, "games_started": None}
...
def attach_probable_pitcher_stats(schedule: list[dict[str, Any]], season: int) -> list[dict[str, Any]]:
    ...
    def get_stats(pitcher_id: int | None) -> dict[str, Any]:
        ...
        if pitcher_id not in cache:
            cache[pitcher_id] = fetch_pitcher_season_stats(pitcher_id, season)
        return cache[pitcher_id]
    enriched: list[dict[str, Any]] = []
    for game in schedule:
        row = dict(game)
        row["away_starter_stats"] = get_stats(game.get("away_probable_pitcher_id"))
        row["home_starter_stats"] = get_stats(game.get("home_probable_pitcher_id"))
        enriched.append(row)
    return enriched
