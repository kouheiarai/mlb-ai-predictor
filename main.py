...
from predictor import build_elo_ratings, fetch_completed_games, make_moneyline_predictions
...
def fetch_odds(api_key: str) -> tuple[list[dict[str, Any]], dict[str, str]]:
    params = {"apiKey": api_key, "bookmakers": "pinnacle", "markets": "h2h,spreads", "oddsFormat": "decimal", "dateFormat": "iso"}
...
def write_report(predictions: list[dict[str, Any]], schedule: list[dict[str, Any]], fetched_at: str, quota: dict[str, str], path: Path) -> None:
    lines = [
    ...
    buys = [row for row in predictions if row["recommendation"] == "BUY"]
    if not buys:
        lines.append("No EV 5%+ Moneyline bets.")
    else:
        for index, row in enumerate(buys, 1):
            lines.extend([
                ...
                f"- Recent 10: {pct(row['recent_10_win_pct'])}",
                f"- Starter quality: {row['starter_quality']:+.2f}",
                "",
            ])
    lines.extend(["## All Predictions", ""])
    for row in predictions:
        lines.append(f"- {row['selection']} | AI {pct(row['model_probability'])} | EV {pct(row['ev'])} | Kelly {pct(row['quarter_kelly'])} | {row['recommendation']}")
...
def main() -> int:
    try:
        ...
        predictions = make_moneyline_predictions(odds_rows, elo_ratings, completed_games, schedule)
        ...
        files = {
            ...
            "predictions.json": {"fetched_at_utc": fetched_at, "predictions": predictions},
        }
        ...
        write_report(predictions, schedule, fetched_at, quota, OUTPUT_DIR / "report.md")
        print(f"Generated {len(predictions)} predictions from {len(odds_events)} odds events.")
        return 0
